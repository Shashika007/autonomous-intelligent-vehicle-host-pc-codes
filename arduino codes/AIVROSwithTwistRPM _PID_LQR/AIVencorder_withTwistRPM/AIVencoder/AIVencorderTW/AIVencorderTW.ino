...
{
  float R1 = gain*msg.data;
  if(R1 > 0)
  {
    
    digitalWrite(Motor2_Start, LOW);
    digitalWrite(Motor2_Direction, LOW);  
    analogWrite(Motor2_Speed,abs(R1));
...
{
  float L2 = gain*msg.data;
  if(L2 < 0)
  {
    
    digitalWrite(Motor1_Start, LOW);
    digitalWrite(Motor1_Direction, LOW);  
    analogWrite(Motor1_Speed,abs(L2));
  }
   else if(L2 > 0)
  {  
    digitalWrite(Motor1_Start, LOW);
...
{
  float L3 = gain*msg.data;
  if(L3 < 0)
  {    
    digitalWrite(Motor4_Start, LOW);
    digitalWrite(Motor4_Direction, LOW); 
    analogWrite(Motor4_Speed,abs(L3)); 
  }
  else if(L3 > 0)
  {  
    digitalWrite(Motor4_Start, LOW);
...
{
  float R4 = gain*msg.data;
  if(R4 > 0)
  {
    
    digitalWrite(Motor3_Start, LOW);
    digitalWrite(Motor3_Direction, LOW); 
    analogWrite(Motor3_Speed,abs(R4)); 
  }
  else if(R4 < 0)
  {  
    digitalWrite(Motor3_Start, LOW);
...
ros::Subscriber<geometry_msgs::Twist> subCmdVel("cmd_vel/lqr", cmdVelAIV);
